...
{
    if(activePeripheral && activePeripheral.state){
        return KONASHI_FAILURE;
    }
...
- (BOOL) _isConnected
{
    return (activePeripheral && activePeripheral.state);
}
...
- (int) _analogWrite:(int)pin milliVolt:(int)milliVolt
{
    if(pin >= AIO0 && pin <= AIO2 && milliVolt >= 0 && milliVolt <= KONASHI_ANALOG_REFERENCE &&
       activePeripheral && activePeripheral.state){
...
- (int) _i2cSendCondition:(int)condition
{
    if((condition == KONASHI_I2C_START_CONDITION || condition == KONASHI_I2C_RESTART_CONDITION ||
       condition == KONASHI_I2C_STOP_CONDITION) && activePeripheral && activePeripheral.state){
        Byte t = condition;
        NSData *d = [[NSData alloc] initWithBytes:&t length:1];
...
{
    if(activePeripheral && activePeripheral.state && uartSetting==KONASHI_UART_DISABLE){
...
{    
    if(activePeripheral && activePeripheral.state && uartSetting==KONASHI_UART_ENABLE){
